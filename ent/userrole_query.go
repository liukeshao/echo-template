...
func (urq *UserRoleQuery) QueryGranter() *UserQuery {
	query := (&UserClient{config: urq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := urq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (urq *UserRoleQuery) Clone() *UserRoleQuery {
	...
	return &UserRoleQuery{
		...
		withGranter: urq.withGranter.Clone(),
...
func (urq *UserRoleQuery) WithGranter(opts ...func(*UserQuery)) *UserRoleQuery {
	query := (&UserClient{config: urq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	urq.withGranter = query
	return urq
}
...
func (urq *UserRoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserRole, error) {
	...
	if query := urq.withGranter; query != nil {
		if err := urq.loadGranter(ctx, query, nodes, nil,
			func(n *UserRole, e *User) { n.Edges.Granter = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (urq *UserRoleQuery) loadGranter(ctx context.Context, query *UserQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*UserRole)
	for i := range nodes {
...
func (urq *UserRoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := urq.ctx.Fields; len(fields) > 0 {
		...
		if urq.withGranter != nil {
			_spec.Node.AddColumnOnce(userrole.FieldGrantedBy)
		}
	}
