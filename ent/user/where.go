...
// HasGrantedRoles applies the HasEdge predicate on the "granted_roles" edge.
func HasGrantedRoles() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasGrantedRolesWith applies the HasEdge predicate on the "granted_roles" edge with a given conditions (other predicates).
func HasGrantedRolesWith(preds ...predicate.UserRole) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newGrantedRolesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
