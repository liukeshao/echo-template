...
type UserRole struct {
	...
	// 状态：active-生效，inactive-暂停，expired-过期
...
type UserRoleEdges struct {
	...
	Granter *User `json:"granter,omitempty"`
...
// was not loaded in eager-loading, or loaded but was not found.
func (e UserRoleEdges) GranterOrErr() (*User, error) {
	if e.Granter != nil {
...
func (*UserRole) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case userrole.FieldID, userrole.FieldUserID, userrole.FieldRoleID, userrole.FieldGrantedBy, userrole.FieldStatus, userrole.FieldRemark:
			values[i] = new(sql.NullString)
		case userrole.FieldCreatedAt, userrole.FieldUpdatedAt, userrole.FieldGrantedAt, userrole.FieldExpiresAt:
	...
	for i := range columns {
		...
		case userrole.FieldGrantedBy:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				ur.GrantedBy = new(string)
				*ur.GrantedBy = value.String
			}
		...
		case userrole.FieldStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				ur.Status = userrole.Status(value.String)
			}
		case userrole.FieldRemark:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field remark", values[i])
			} else if value.Valid {
				ur.Remark = new(string)
				*ur.Remark = value.String
			}
		default:
...
// QueryGranter queries the "granter" edge of the UserRole entity.
func (ur *UserRole) QueryGranter() *UserQuery {
	return NewUserRoleClient(ur.config).QueryGranter(ur)
}
