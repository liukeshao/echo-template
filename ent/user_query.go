...
type UserQuery struct {
	...
	withGrantedRoles *UserRoleQuery
...
// QueryGrantedRoles chains the current query on the "granted_roles" edge.
func (uq *UserQuery) QueryGrantedRoles() *UserRoleQuery {
	query := (&UserRoleClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withGrantedRoles: uq.withGrantedRoles.Clone(),
...
// WithGrantedRoles tells the query-builder to eager-load the nodes that are connected to
// the "granted_roles" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithGrantedRoles(opts ...func(*UserRoleQuery)) *UserQuery {
	query := (&UserRoleClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withGrantedRoles = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withGrantedRoles; query != nil {
		if err := uq.loadGrantedRoles(ctx, query, nodes,
			func(n *User) { n.Edges.GrantedRoles = []*UserRole{} },
			func(n *User, e *UserRole) { n.Edges.GrantedRoles = append(n.Edges.GrantedRoles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadGrantedRoles(ctx context.Context, query *UserRoleQuery, nodes []*User, init func(*User), assign func(*User, *UserRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userrole.FieldGrantedBy)
	}
	query.Where(predicate.UserRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.GrantedRolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
