...
// QueryDepartmentRel chains the current query on the "department_rel" edge.
func (uq *UserQuery) QueryDepartmentRel() *DepartmentQuery {
...
// WithDepartmentRel tells the query-builder to eager-load the nodes that are connected to
// the "department_rel" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithDepartmentRel(opts ...func(*DepartmentQuery)) *UserQuery {
	query := (&DepartmentClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) loadDepartmentRel(ctx context.Context, query *DepartmentQuery, nodes []*User, init func(*User), assign func(*User, *Department)) error {
